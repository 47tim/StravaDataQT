...
// Tim Hyde
// 11.11.2021 -- improved readability
// 12.7.2021 -- widened scale for heartrate graphs

// To-Do:
// On the right, add top stats. Furthest run, highest max hr, etc...
...
void MainWindow::readStrings(){
    ...
    // Reading in files to vectors storing total time states
...
void MainWindow::setStats(){
    ...
    QString valueAsString6 = QString::number(qRound(avg_elevation * 3.28));
...
void MainWindow::plot1(){
    ...
    QVector<double> v_distance;   // QCustomPlot only accepts vectors, so converting from list
...
void MainWindow::plot2(){
    ...
    ui->widget2->yAxis->setRange(140, 210);
    ui->widget2->replot();
...
void MainWindow::plot3(){
    ...
    for (int i = 0; i < MAX_HEARTRATE.size(); i++){
     ...
     ui->widget3->yAxis->setRange(100, 210);
     ui->widget3->replot();
...
void MainWindow::setDay(){ // only used if the day mode is selected
    ...
    label6text.push_back(QString::number(qRound(total_elevation_gain[lastNum3 - day] * 3.28)));
    ui->label6->setText(label6text);
    ui->label_eg->setText("Elevation Gain (ft)");
